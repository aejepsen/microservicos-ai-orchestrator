"""Auth interna fail-closed, rate-limit por IP, guarda anti-SSRF."""
...
def validate_outbound_url(url: str, allow_local: bool) -> None:
    """Anti-SSRF: só http/https; bloqueia metadata/loopback salvo opt-in."""
    parsed = urlparse(url)
    if parsed.scheme not in {"http", "https"}:
        raise HTTPException(status_code=422, detail="esquema de URL nao permitido")
    ...
    try:
        ...
        raise HTTPException(status_code=424, detail=f"host nao resolvido: {host}") from exc
    for info in infos:
        ip = ipaddress.ip_address(info[4][0])
        if ip.is_private or any(ip in net for net in _BLOCKED_NETS):
            raise HTTPException(
                status_code=422, detail="URL aponta para rede interna (SSRF bloqueado)"
            )
