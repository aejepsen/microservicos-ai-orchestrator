"""Auth interna fail-closed + rate-limit por IP + guarda anti-SSRF (downstream URLs)."""
...
def validate_outbound_url(url: str, allow_local: bool) -> None:
    """Anti-SSRF do downstream URL: só http/https; bloqueia metadata/loopback salvo opt-in."""
    parsed = urlparse(url)
