...
def client_ip(request: Request) -> str:
    """Cadeia de confiança: CF-Connecting-IP → X-Real-IP → X-Forwarded-For → socket."""
...
def verify_internal_key(request: Request, settings: Settings) -> None:
    """Fail-closed: sem INTERNAL_KEY configurada, só ALLOW_OPEN_ACCESS=1 libera."""
...
class RateLimiter:
    """Sliding window por IP; max_entries + eviction contra memory exhaustion."""
