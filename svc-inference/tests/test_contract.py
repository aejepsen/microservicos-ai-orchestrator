...
def test_all_paths_exercised(client, auth_headers) -> None:
    ...
    checks = {
        ("post", "/v1/chat/completions"): lambda: client.post("/v1/chat/completions", json=body, headers=auth_headers),
        ("get", "/v1/models"): lambda: client.get("/v1/models", headers=auth_headers),
